...
export async function GET(request: NextRequest) {
  try {
    ...
    if (orderId) {
      ...
      if (financialStatus) queryParams += `&financial_status=${financialStatus}`;
      
      url = `https://${shop}/admin/api/2024-10/orders.json?${queryParams}`;
    }
    ...
    return NextResponse.json({
      success: true,
      data: orderId ? data.order : data.orders,
